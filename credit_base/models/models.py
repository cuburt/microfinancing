...
class LoanClient(models.Model):
    ...
    def write(self, values):
        print(values)
        try:
            if ('is_company' in values and not values['is_company']) and ('company_id' in values and values['company_id']):
                print('STEP 1')
                print(values)
...
class ResBranch(models.Model):
    ...
    def create(self, values):
        try:
            ...
            try:
                cash_receipt = self.env['account.journal'].sudo().create({
                ...
                print(cash_receipt.name+' created for company '+company.name)
